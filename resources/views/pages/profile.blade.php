...
@section('content')
    <div class="min-h-screen pt-36 pb-24 w-11/12 mx-auto">
        ...
        <div class="shadow-xl relative mx-auto rounded grid lg:grid-cols-3 grid-cols-1">
            ...
            <div class="lg:col-span-1 pt-10 bg-dy flex flex-col justify-between gap-8 rounded-l text-center text-white relative"
                ...
                <button type="button" class="absolute top-4 right-4 text-white hover:text-yellow transition duration-300"
                    ...
                    <div class="flex flex-col gap-2">
                        ...
                        <select id="gender" name="gender_id" class="w-full px-4 py-2 border rounded-md text-black">
                            @foreach ($genders as $gender)
                                <option value="{{ $gender->id }}" {{ $gender->id === $user->gender->id ?? 'selected' }}>
                                    {{ $gender->name }}</option>
                            @endforeach
                        </select>
                        @error('gender')
                        ...
                        @if ($top_post)
                            <div class="bg-white p-4 rounded-md shadow-md w-full">
                                <div class="flex items-center mb-4">
                                    ...
                                    <span
                                        class="text-black font-semibold">{{ $top_post->is_anonymous === 0 ? $top_post->user->first_name . ' ' . $top_post->user->last_name : 'Anonymous' }}</span>
                                </div>
