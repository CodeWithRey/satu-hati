    ...
    <div class="bg-white min-h-screen min-w-screen grid lg:grid-cols-2 grid-cols-1">
        ...
        <div class="bg-yellow lg:p-10 rounded shadow-md flex flex-col justify-center ">
            ...
            <form method="POST" action="{{ route('login') }}" class="bg-white p-8 rounded-lg w-11/12 mx-auto">
                ...
                <h2 class="text-md mb-4 flex items-center justify-center text-center">Kami senang melihat Anda kembali! Harap masukkan informasi login Anda.</h2>
                <!-- Isian Form -->
                <div class="flex flex-col gap-2 mb-4">
                    <label for="email" class="block text-sm font-semibold text-gray-600">E-Mail</label>
                    <input type="email" placeholder="Masukkan E-Mail anda..." id="email" value="{{ old('email') }}" name="email"
                ...
                <!-- Tombol Login -->
                <button type="submit" class="bg-dy hover:bg-dy-dark transition duration-300 focus:ring-2 focus:ring-orange-300 w-full text-white px-10 py-2 rounded-md">Masuk</button>


            </form>
            <div class="text-center mb-1"><br>
                <p class="small mb-0">Belum punya akun? <a href="{{ route('register') }}" class="text-dy font-bold hover:underline hover:text-dy-dark underline-offset-2 transition duration-300">Buat
                        akun</a></p>
            </div>
